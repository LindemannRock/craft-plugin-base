...
LindemannRock Base includes full translations for 12 languages out of the box.
...
You can override any translation string by creating a static translation file in your project. **The filename must match the translation category — which is the plugin handle, `lindemannrock-base` (not `base`).** A file named `base.php` is silently ignored:

```
translations/
└── de/
    └── lindemannrock-base.php
```
...
1. Add a new translation category using the plugin handle (`lindemannrock-base`)
...
If you find a translation error or want to improve a translation, please [open an issue](https://github.com/LindemannRock/craft-plugin-base/issues) with:
