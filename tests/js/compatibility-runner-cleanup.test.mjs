...
function fixture({
    ...
    writeFileSync(path.join(fixturePackageRoot, 'composer.json'), JSON.stringify({
        ...
        type: 'yii2-module',
        extra: {handle: 'lindemannrock-base'},
    }));
...
printf 'composer:%s\\n' "$*" >> "$BASE_COMPAT_TEST_LOG"
if [[ "$1" == "create-project" ]]; then
  project_dir="$3"
  mkdir -p "$project_dir"
  printf '{"require-dev":{"fixture":"1"}}\\n' > "$project_dir/composer.json"
  if [[ "$BASE_COMPAT_WAIT_CREATE" == "1" ]]; then
    trap 'exit 143' TERM
    while :; do sleep 1; done
  fi
fi
if [[ -n "$BASE_COMPAT_FAIL_MATCH" && "composer $*" == *"$BASE_COMPAT_FAIL_MATCH"* ]]; then exit 41; fi
exit 0
`);
...
if [[ "$1" == "config" ]]; then
  ...
  printf '%s\\n' "$project_name" > "$BASE_DDEV_RESOURCE_ROOT/current-project"
  mkdir -p "$BASE_DDEV_RESOURCE_ROOT/$project_name"
fi
if [[ "$1" == "delete" ]]; then
  project_name="\${@: -1}"
  if [[ "$BASE_COMPAT_CLEANUP_EXIT" -ne 0 ]]; then exit "$BASE_COMPAT_CLEANUP_EXIT"; fi
  rm -rf "$BASE_DDEV_RESOURCE_ROOT/$project_name"
  rm -f "$BASE_DDEV_RESOURCE_ROOT/current-project"
  exit 0
fi
...
`);
    ...
    const environment = {
        ...
        BASE_COMPAT_TEST_LOG: logPath,
        BASE_COMPAT_FAIL_MATCH: failureMatch,
...
test('install mode preserves the Yii module smoke path and cleans every partial DDEV path', async (context) => {
    for (const [name, failureMatch, expected] of [
        ['success', '', 0],
        ['config failure', 'ddev config ', 41],
        ['start failure', 'ddev start', 41],
        ['Craft install failure', 'ddev craft install', 41],
        ['module smoke failure', 'ddev exec env', 41],
    ]) {
        await context.test(name, () => {
